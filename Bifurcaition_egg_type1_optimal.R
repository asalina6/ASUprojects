  ...
  #Initial set up
  #########################################
  #eggvector=seq(.58,2.42,.04);
  eggvector=seq(.58,2.42,.07);
  egg=numeric(0)
  dw=.0025;
  dg=.001;
  vJ=numeric(0)
  ...
  BNbestvector=seq(0,length(eggvector)-1)
  BGbestvector=seq(0,length(eggvector)-1)
  deltaNbestvector=seq(0,length(eggvector)-1)
  deltaGbestvector=seq(0,length(eggvector)-1)
  #finding indices
  ...
  for( i in 1:length(eggvector))
  {
    ...
    for(j in 1:length(x))
    {
      ...
      if(round(Nout[j],1)==round(gout[j],1)) #used to be 2
      {
        vJ=j
      }
    }
  ...
  }#This ends the egg vector loop
  text_main = paste("Bifurcation Type 1 with Sensitivity: e")
  mult.fig(6,main=text_main,oma=c(1,2,4,1))
  plot(eggvector,Nstar, main="Bifurcation Type 1: e vs N*",xlab="e",ylab="N*")
 
  plot(eggvector,Xstar,main="Bifurcation Type 1: e vs X*",xlab="e",ylab="X*")
  
  plot(eggvector,BNbestvector, main=bquote(plain("Bifurcation Type 1: e vs Optimal fit ")*beta*plain("n")*plain(" value")),xlab="e",ylab=expression(paste(beta,"n")))

  plot(eggvector,BGbestvector, main=bquote(plain("Bifurcation Type 1: e vs Optimal fit ")*beta*plain("g")*plain(" value")),xlab="e",ylab=expression(paste(beta,"g")))

  plot(eggvector,deltaGbestvector, main=bquote(plain("Bifurcation Type 1: e vs Optimal fit ")*delta*plain("n")*plain(" value")),xlab="\n e",ylab=expression(paste(delta,"n")))
 
  plot(eggvector,deltaNbestvector, main=bquote(plain("Bifurcation Type 1: e vs Optimal fit ")*delta*plain("g")*plain(" value")),xlab="e",ylab=expression(paste(delta,"g")))
